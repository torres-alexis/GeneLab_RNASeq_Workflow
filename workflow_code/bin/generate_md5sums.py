...
def main():
    ...
    raw = args.raw or not (args.raw or args.processed)
    processed = args.processed or not (args.raw or args.processed)

    raw_md5_file = f"raw_md5sum{args.assay_suffix}.tsv"
    processed_md5_file = f"processed_md5sum{args.assay_suffix}.tsv"

    if raw:
        open(raw_md5_file, 'w').close()
    if processed:
        open(processed_md5_file, 'w').close()
